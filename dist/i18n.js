...
export function translateMessage(message) {
    ...
    if (exact !== message || currentLanguage === "en")
        return exact;
    ...
    if (unknownSlot)
        return `Bilinmeyen alan kimliği: ${unknownSlot[1]}.`;
    const invalidComponent = message.match(/^Invalid component ID for (.+)\.$/);
    if (invalidComponent)
        return `${t(invalidComponent[1])} için geçersiz bileşen kimliği.`;
    const incompatible = message.match(/^(.+) is not compatible with (.+)\.$/);
    if (incompatible)
        return `${t(incompatible[1])}, ${t(incompatible[2])} ile uyumlu değil.`;
    return message;
}
export function applyPageTranslations() {
    ...
    document.querySelectorAll("[data-source-message]").forEach(element => {
        element.textContent = translateMessage(element.dataset.sourceMessage ?? "");
    });
    document.querySelectorAll("[data-source-messages]").forEach(element => {
        try {
            const messages = JSON.parse(element.dataset.sourceMessages ?? "[]");
