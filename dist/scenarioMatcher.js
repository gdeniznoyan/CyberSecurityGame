...
export const predefinedArchitectures = [
    {
...
function selectedBySection() {
    ...
    for (const state of getSlotStates()) {
        ...
        if (section)
            selected[section].push(state.componentId);
    }
    return selected;
}
...
export function analyzeArchitecture() {
    ...
    const match = predefinedArchitectures.find(scenario => Object.keys(scenario.components)
        .every(section => sameComponents(current[section], scenario.components[section])));
    return match
        ? {
...
export function renderArchitectureResult(result) {
    ...
    if (!name || !level || !score || !description || !hint)
        return;
...
//# sourceMappingURL=scenarioMatcher.js.map