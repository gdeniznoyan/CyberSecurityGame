...
export function evaluateSecurityAnalysis() {
    const selectedIds = new Set(getPlacements().map((placement) => placement.componentId));
    const items = [];
    componentList
    ...
    addSpecialAreaAnalysis("third-party-services", "third-party-dependencies", "No Third-Party Dependency: Critical security functions remain under organizational control without introducing additional external trust dependencies.", selectedIds, items);
    addSpecialAreaAnalysis("protected-application", "target-application", "No Target Selected: No protected application has been selected, so the architecture does not yet have a valid access destination.", selectedIds, items);
    return items;
}
function addSpecialAreaAnalysis(areaId, group, emptyOutput, selectedIds, items) {
    const areaComponents = componentList.filter((component) => component.area === areaId && component.importance === "special");
    const selectedComponents = areaComponents.filter((component) => selectedIds.has(component.id));
    if (selectedComponents.length) {
        selectedComponents.forEach((component) => items.push({
            group,
...
export function getTotalScore(evaluations) {
    ...
    const finalScore = Math.round(weightedSecurityScore + thirdPartyPercentage * 0.1);
...
function meetsPostZeroTrustRequirements(evaluations) {
    ...
    const hasProtectedApplication = postZeroTrustProtectedApplications.some((id) => selectedIds.has(id));
    return (thirdPartyIsEmpty &&
